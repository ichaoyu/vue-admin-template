 ...
 */
// 获取消息列表
export const getMessageListApi = (params: any): Promise<PageVO<MessageType>> => {
...
// 获取消息详情
export const getMessageDetailApi = (id: number | string): Promise<MessageType> => {
  return request.get({ url: `/api/cms/message/${id}` });
};

// 回复消息 - 这个API在NestJS后端没有找到，暂时保留原路径
export const replyMessageApi = (data: { id: number | string; reply: string }): Promise<any> => {
