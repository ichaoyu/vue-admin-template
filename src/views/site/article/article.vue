...
// 编辑
const currentRecord = ref<ArticleType | null>(null);
const edit = (row: ArticleType) => {
  currentRecord.value = row;
  toggleShowDraw(true);
};
// #region 删除
const onDelete = async (row: ArticleType) => {
  const ids = [row.id!];
  handleDelete(ids);
};
// 批量删除
const onBatchDelete = (data: ArticleType[]) => {
...
const onSelection = (data: ArticleType[]) => {
