    ...
    <template #default>
      ...
      >
        <el-form-item label="字典标签" prop="dictName">
          <el-input v-model="ruleForm.dictName" placeholder="请输入字典标签" />
        </el-form-item>
        <el-form-item label="字典键值" prop="dictType">
          <el-input v-model="ruleForm.dictType" placeholder="请输入字典键值" />
        </el-form-item>
...
import { addDictTypeApi, updateDictTypeApi } from '@/api/system.dict.api';
...
const props = defineProps({
  ...
  record: {
    type: Object as PropType<DictTypeType | null>,
    default: () => null,
  },
});
...
const ruleForm = ref<DictTypeType>({
  dictName: '',
  dictType: '',
  remark: '',
});

const rules = reactive<FormRules<DictTypeType>>({
  // title: [
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate(async (valid) => {
    if (valid) {
      try {
        loading.value = true;
        const saveApi = props.record ? updateDictTypeApi : addDictTypeApi;
