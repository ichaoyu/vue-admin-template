...
import { LoginLogType } from '@/interface';
import { ElMessage, ElMessageBox } from 'element-plus';

const { meta } = useRoute();
// #region 删除
const onDelete = async (row: LoginLogType) => {
  const ids = [row.id!];
  handleDelete(ids);
};
// 批量删除
const onBatchDelete = (data: LoginLogType[]) => {
...
const fetchTableList = async () => {
  ...
  try {
    ...
    const res = await getLoginLogApi(params);
    data.value = res.list;
...
const onSelection = (data: LoginLogType[]) => {
